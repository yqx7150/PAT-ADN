...
class NatureImage(torch.utils.data.Dataset):
    def __init__(self, a_dir="data/train/nature_image/artifact", b_dir="data/train/nature_image/no_artifact",
        random_flip=True, load_size=256, crop_size=256, crop_type="random"):#load_size=384, crop_size=256, 
    ...
    def get(self, a_file):
        ...
        a = self.transform(Image.open(a_file).convert("RGB"))#RGB
        b = self.transform(Image.open(choice(self.b_files)).convert("RGB"))#RGB
...
if __name__ == "__main__":
    ...
    #np.save('/media/lqg/109公用盘/adn-master/runs/a.npy', a)
    #assert 0
    plt.ion()
